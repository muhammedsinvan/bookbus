...
const UserManagment = () => {
  ...
  useEffect(()=>{
    (async()=>{
      try{
let data = await axios.get('/admin/getalluser',config)
setdata(data.data)
      }catch(error){
        console.log(error)
      }
...
>
  ...
  <TableCell><Button variant="contained" value={res._id} onClick={e => viewbooking(e.target.value)}>View Bookings</Button></TableCell>
</TableRow>
     ))}      
